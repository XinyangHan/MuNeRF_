...
# from facemorpher.warper import warp_image
import PIL.Image
...
def blendP(img_target, img_source, img_mask, offset=(0, 0)):
    ...
    img_mask = img_mask[region_source[0]:region_source[2], region_source[1]:region_source[3]].squeeze()
...
class FaceCropper():
    ...
    def warpFace(self, before_img, after_img,
                color,
                 ...
                 adjust_value = False,
                 mask_path = "/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/res/test_res1/1045.png"
                ):
        before_shape = before_img.shape
        after_shape = after_img.shape
        if use_stasm:
        ...
        set = {0,7,8,9,14,15,16,17,18}
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                for k in range(mask.shape[2]):
                    if mask[i][j][k] in set:
                        mask[i][j][k] = 0
                    else:
                        mask[i][j][k] = 1
        ...
        if use_tps:
            ...
            try:
                ...
                cv2.imwrite("/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/after_t.png", after_transformed)
        ...
        if a:
            ...
            black_mask = after_transformed.astype(np.float).sum(axis=2) == 0
            #cv2.imwrite('./before_img.jpg', before_img)
            # black_mask = np.logical_or(black_mask, before_img.astype(np.float).sum(axis=2) == 0)
            black_mask = np.logical_or(black_mask, mask.astype(np.float).sum(axis=2) == 0)
            ...
            #cv2.imwrite('./debug_poisson.jpg', poisson_face)
            ...
            if additional_gaussian:
                ...
                result_img = before_img * (black_mask) + after_transformed * face_mask
                cv2.imwrite('./result_img.jpg', result_img)

            raw_img = "/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/raw_img.png"
            cv2.imwrite(raw_img, result_img)

            for x in range(result_img.shape[0]):
                for y in range(result_img.shape[1]):
                    for z in range(result_img.shape[2]):
                        if (after_transformed[x][y][z] == 0) and (mask[x][y][z] > 0):
                            # pdb.set_trace()
                            result_img[x][y][z] = color[z]

            cv2.imwrite("/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/result_img.png", result_img)
            cv2.imwrite("/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/after_transform.png", after_transformed)
            cv2.imwrite("/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/face_mask.png", face_mask.astype(np.uint8)*255)


            #transform the image into PIL form
            # result_img = np.asarray(Image.fromarray(cv2.cvtColor(result_img,cv2.COLOR_BGR2RGB)))
            # after_transformed = np.asarray(Image.fromarray(cv2.cvtColor(after_transformed,cv2.COLOR_BGR2RGB)))

            demo_mask = "/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicpoissonblending-master/testimages/test1_mask.png"
            demo_source = "/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicpoissonblending-master/testimages/test1_src.png"
            demo_target = "/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicpoissonblending-master/testimages/test1_target.png"

            current_mask = '/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/face_mask.png'
            current_source = '/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/after_transform.png'
            current_target = '/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/result_img.png'

            img_mask = np.asarray(PIL.Image.open(current_mask))
            img_mask.flags.writeable = True
            img_source = np.asarray(PIL.Image.open(current_target))
            img_source.flags.writeable = True
            img_target = np.asarray(PIL.Image.open(current_target))
            img_target.flags.writeable = True

            '''Validate the blendP '''

            # img_mask = np.asarray(PIL.Image.open(demo_mask))
            # img_mask.flags.writeable = True
            # img_source = np.asarray(PIL.Image.open(demo_target))
            # img_source.flags.writeable = True
            # img_target = np.asarray(PIL.Image.open(demo_target))
            # img_target.flags.writeable = True


            '''Validate the blendP '''
            img_ret = blendP(img_target, img_source, img_mask)

            # 

            img_ret = PIL.Image.fromarray(np.uint8(img_ret))
            img_ret.save('/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/res.png')

            # pdb.set_trace()

            result_img = cv2.imread('/data/hanxinyang/MuNeRF_latest/process_data_hy/makeup-transfer-publicface-parsing.PyTorch-master/debugH/P/res.png')


            # pdb.set_trace()

            # result_img.flags.writeable = True
            # after_transformed.flags.writeable = True
            # face_mask.flags.writeable = True

            # result_img = blendP(result_img, after_transformed, face_mask, offset=(40,-30))
            ...
            return result_img, result_points, True
        else:
