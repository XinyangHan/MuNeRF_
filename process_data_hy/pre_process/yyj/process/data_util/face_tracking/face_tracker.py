...
for focal in range(600, 1500, 100):
    ...
    #iterate 2000 times to predict the pose, euler_angle and trans
    ...
    #iterate 2500 to extract the id and expression
...
for i in range(int((num_frames-1)/batch_size+1)):
    ...
    for j in range(sel_ids.shape[0]):
        img_arr = render_proj[j, :, :, :3].byte().detach().cpu().numpy()[
            :, :, ::-1]
        cv2.imwrite(os.path.join(debug_render_dir, str(sel_ids[j]) + '.png'),
                    img_arr)
