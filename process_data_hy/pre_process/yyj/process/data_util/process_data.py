...
parser.add_argument('--dataset', type=str,
                    default='dataset', help='dataset dir')
...
id = args.id
# vid_file = os.path.join(args.dataset, 'vids', id+'.mp4')
vid_file = os.path.join(args.dataset,  id+'.mp4')
...
id_dir = os.path.join(args.dataset, id)
Path(id_dir).mkdir(parents=True, exist_ok=True)
ori_imgs_dir = os.path.join(args.dataset, id, 'ori_imgs')
...
if running_step == 1:
    ...
    while(True):
        ...
        if frame is None:
            break
        cv2.imwrite(os.path.join(ori_imgs_dir, str("%04d"%frame_num) + '.png'), frame)
...
for i in range(max_frame_num):
    # pdb.set_trace()
    if os.path.isfile(os.path.join(ori_imgs_dir, '%04d'%i + '.lms')):
        valid_img_ids.append('%04d'%i)
...
if running_step == 3:
    ...
    face_parsing_cmd = 'python data_util/face_parsing/test.py --respath=' + id_dir + \
        '/parsing --imgpath=' + ori_imgs_dir
...
if running_step == 4:
    ...
    cv2.imwrite(os.path.join(id_dir, 'bc.png'), bc_img)
...
if running_step == 5:
    ...
    for i in valid_img_ids:
        ...
        cv2.imwrite(os.path.join(com_imgs_dir, str(i) + '.png'), img)
        img[~head_part] = bc_img[~head_part]
        cv2.imwrite(os.path.join(head_imgs_dir, str(i) + '.png'), img)
...
if running_step == 6:
    ...
    est_pose_cmd = 'python data_util/face_tracking/face_tracker.py --idname=' + \
        ...
        ' --frame_num=' + str(max_frame_num) + ' --dataset=' + args.dataset
...
if running_step == 7:
    ...
    val_test_ids = list(np.arange(5, valid_num, 5))
    train_ids = list(set(np.arange(valid_num)).difference(set(val_test_ids)))
    val_ids = [val_test_ids[i] for i in range(0,len(val_test_ids),2)]
    test_ids = [val_test_ids[i] for i in range(1,len(val_test_ids),2)]
    ...
    for ii in range(len(train_val_ids)):
        transform_dict = dict()
        ...
        ids = train_val_ids[ii]
        save_id = save_ids[ii]
        os.makedirs(os.path.join(ori_imgs_dir, save_id), exist_ok=True)
        for i in ids:
            # i = i.item()
            shutil.copyfile(os.path.join(ori_imgs_dir, valid_img_ids[i]+'.png'), os.path.join(ori_imgs_dir, save_id, valid_img_ids[i]+'.png'))
            frame_dict = dict()
            ...
            frame_dict['expression'] = params_dict['exp'][i].numpy().tolist()
