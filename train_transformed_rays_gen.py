...
def main():
    ...
    if train_latent_codes:
        ...
        #latent_codes = torch.zeros(911,32, device=device)
    ...
    if os.path.exists(configargs.load_checkpoint) and not density:
        ...
        flag_density = True
        if flag_density: 
            checkpoint_d = torch.load(configargs.load_density_checkpoint, map_location=device)
        ...
        #torch.save(checkpoint_density, os.path.join(logdir, "checkpoint_density_fixed" + ".ckpt"))
            ...
            old_checkpoint_coarse_d, old_checkpoint_fine_d = {}, {}
            for item in density_train:
                old_checkpoint_coarse_d = checkpoint_d["model_coarse_state_dict"]
                old_checkpoint_fine_d = checkpoint_d["model_fine_state_dict"]
        ...
        if checkpoint["latent_codes"] is not None:
            ...
            latent_codes = torch.nn.Parameter(latent_codes.to(device))
            if not disable_latent_codes:
                trainable_parameters.append(latent_codes)
                latent_codes.requires_grad = True
        if use_remove_loss:
    ...
    for i in trange(start_iter, train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            if not disable_latent_codes:
                latent_code = latent_codes[0].to(device) if train_latent_codes else None  #here
            else:
            ...
            if not density and cfg.loss.addloss.loss and i % cfg.loss.addloss.iter == 0:
                ...
                if_points, if_texture, if_uv = False, True, False
