    ...
    for i in trange(start_iter, train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            if not density and cfg.loss.addloss.loss and i % cfg.loss.addloss.iter == 0:
                ...
                # mask_makeup_np = mask_makeup.cpu().numpy()
                # mask_nonmakeup_np = mask_nonmakeup.cpu().numpy()
                # print(np.where(mask_makeup_np==4))
                # print(np.where(mask_nonmakeup_np==4))
                ...
                if (not if_dense) :
                # if (not if_dense) and ("boy4" not in configargs.debug_dir):
                # if not if_dense:
