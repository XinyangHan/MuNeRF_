...
def run_network(network_fn, pts, ray_batch, chunksize, embed_fn, embeddirs_fn, expressions = None, lights = None, details = None, latent_code = None):
    ...
    #forward(self, x,  expr=None, lights=None, latent_code=None, **kwargs):
    #print('run_network: latent_code', latent_code)
    if expressions is None:
        preds = [network_fn(batch) for batch in batches]
    elif latent_code is not None:
        preds = [network_fn(batch, expressions, lights, details, latent_code) for batch in batches]
    else:
        preds = [network_fn(batch, expressions, lights, details, latent_code) for batch in batches]
