...
class ConditionalBlendshapePaperNeRFModel(torch.nn.Module):
    ...
    def __init__(
        ...
        include_input_xyz = 3 if include_input_xyz else 0
        include_input_dir = 3 if include_input_dir else 0
        include_expression = 50 if include_expression else 0
        include_lights = 9 if include_lights else 0
        include_details = 128
        ...
        self.dim_lights = include_lights
        self.dim_details = include_details

        self.layers_xyz = torch.nn.ModuleList()
        self.use_viewdirs = use_viewdirs
        #print('!!!!!!', self.dim_xyz + self.dim_expression + self.dim_latent_code + self.dim_lights)
        self.layers_xyz.append(torch.nn.Linear(self.dim_xyz + self.dim_expression + self.dim_latent_code+ self.dim_lights , 256)) # + self.dim_details
        for i in range(1, 6):
            if i == 3:
                self.layers_xyz.append(torch.nn.Linear(self.dim_xyz + self.dim_expression + self.dim_latent_code + self.dim_lights + 256, 256)) # + self.dim_details 
            else:
                self.layers_xyz.append(torch.nn.Linear(256, 256))
        self.fc_feat = torch.nn.Linear(256, 256)
        self.fc_alpha = torch.nn.Linear(256, 1)
    ...
    def forward(self, x,  expr=None, lights=None, details=None, latent_code=None, **kwargs):
        xyz, dirs = x[..., : self.dim_xyz], x[..., self.dim_xyz :]
        x = xyz#self.relu(self.layers_xyz[0](xyz))
        latent_code = latent_code.repeat(xyz.shape[0], 1)
        lights_encoding = lights.repeat(xyz.shape[0], 1)
        #detail_encoding = details.repeat(xyz.shape[0], 1)
        if self.dim_expression > 0:
            ...
            initial = torch.cat((xyz, expr_encoding, lights_encoding, latent_code), dim=1) # detail_encoding
        ...
        for i in range(1, 3):
            ...
            x = self.relu(x)
        rgb = self.fc_rgb(x)
        #print('after fc_rgb',rgb.shape) #([2048, 3])
        return torch.cat((rgb, alpha), dim=-1)
