...
def _predict_vertices(param, roi_bbox, dense, transform=True):
    ...
    vertex, parsed_param = reconstruct_vertex(param, dense=dense)
    p,offset,alpha_shp,alpha_exp = parsed_param
    ...
    # vertex[2, :] *= s
    info_dict = {}
    # info_dict["vertices"] = vertex
    info_dict['bbx'] = np.array([sx, sy, ex, ey])
    info_dict["p"] = p
    info_dict["offset"] = offset
    info_dict["expression"] = alpha_exp
    info_dict["shape"] = alpha_shp

    info_dict["scale"] = np.array([scale_x, scale_y, s])
    info_dict["px_py"] = np.array([sx, sy])

    # print("px:", sx, " py:", sy)
    # print("scale_x:", scale_x, " scale_y:", scale_y, " scale_z:", s)
    
    return vertex, info_dict
