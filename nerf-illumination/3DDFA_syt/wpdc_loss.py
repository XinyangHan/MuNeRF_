...
class WPDCLoss(nn.Module):
    ...
    def reconstruct_and_parse(self, input_, target):
        # reconstruct
        param = input_ * self.param_std + self.param_mean
    ...
    def _calc_weights_resample(self, input_, target_):
        ...
        ssshp = w_shp_base @ alpha_shp

        #alpha_exp = torch.tensor(alpha_exp, dtype=torch.float).cuda()
        w_exp_base = torch.tensor(w_exp_base, dtype=torch.float).cuda()
        eeexp = w_exp_base @ alpha_exp.cuda()
        
        tmpv = (u_base + ssshp + eeexp).view(N, -1, 3).permute(0, 2, 1)
