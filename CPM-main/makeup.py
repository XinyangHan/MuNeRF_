...
class Makeup:
    def __init__(self, args):
        ...
        self.color = net.Generator_branch(64, 6).to(self.device)
        self.color.load_state_dict(torch.load(args.checkpoint_color, map_location=self.device))
        self.color.eval()
        if args.prn:
    ...
    def get_mask(self, img):
        ...
        x_tensor = torch.from_numpy(x_tensor).unsqueeze(0).to(self.device)
    ...
    def makeup(self, img_A, img_B):
        ...
        img_A = img_A[None, :, :, :]
        img_B = img_B[None, :, :, :]
        real_org = to_var(img_A).to(self.device)
        real_ref = to_var(img_B).to(self.device)
