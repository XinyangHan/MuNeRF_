...
# new functions from hy
def img2mse(img_src, img_tgt):
    return torch.nn.functional.mse_loss(img_src, img_tgt)
    
def landmark2d_loss(landmark_gt, landmark_fine, landmark_coarse):
    return 0.7 * torch.nn.functional.mse_loss(landmark_gt, landmark_fine) + \
            0.3 * torch.nn.functional.mse_loss(landmark_gt, landmark_coarse)

def make_test_model(model, model2, device):
