...
class NeRFModelColormodule(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(3):
            self.layers_dir.append(torch.nn.Linear(128, 128))
        self.fc_rgb = torch.nn.Linear(128, 64)  ##here!!!!!!!!
        self.relu = torch.nn.functional.relu
...
class ResnetGenerator_newupsample(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        model_g = [torch.nn.ReflectionPad2d(3),
            torch.nn.Conv2d(3, ngf, kernel_size=7, padding=0, bias=use_bias),
            norm_layer(ngf),
...
class ResnetGenerator_newupsample2(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        upsample_patch1 = []
        upsample_patch1 = [torch.nn.ConvTranspose2d(33, int(ngf/4),
                                        kernel_size=3, stride=2,
                                        padding=1, output_padding=1,
                                        bias=use_bias),
                    ...
                    torch.nn.ReLU(True)]
        upsample_patch2 = []
        upsample_patch2 = [torch.nn.ConvTranspose2d(int(ngf/4), int(ngf/4),
                                        kernel_size=3, stride=2,
                                        padding=1, output_padding=1,
                                        bias=use_bias),
                    norm_layer(int(ngf/4)),
        ...
        conv_model = [torch.nn.ReflectionPad2d(3),
            torch.nn.Conv2d(img_input_nc*2, int(ngf/4), kernel_size=7, padding=0, bias=use_bias),
            norm_layer(int(ngf/4)),
            torch.nn.ReLU(True)]
        
        conv_model_s = []
        conv_model_s = [torch.nn.ReflectionPad2d(3),
            torch.nn.Conv2d(img_input_nc, int(ngf/4), kernel_size=7, padding=0, bias=use_bias),
            norm_layer(int(ngf/4)),
            torch.nn.ReLU(True)]

        #convstyle = []
        #convstyle = [torch.nn.ReflectionPad2d(3),
        #    torch.nn.Conv2d(img_input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
        #    norm_layer(ngf),
        #    torch.nn.ReLU(True)]
        ...
        for i in range(n_upsampling):  # add upsampling layers
            #mult = 2 ** (n_upsampling - i) # 2 1 
            upsample_model1 += [torch.nn.ConvTranspose2d(int(ngf*4), int(ngf*4),
                                         kernel_size=7, stride=1,
                                         padding=0, output_padding=0,
                                         bias=use_bias),
                      norm_layer(int(ngf*4)),
        ...
        #for i in range(n_downsampling):  # add upsampling layers
        upsample_model2 = [torch.nn.ConvTranspose2d(512, ngf*4,
                                        kernel_size=3, stride=2,
                                        padding=1, output_padding=1,
                                        bias=use_bias),
                    norm_layer(ngf*4),
        ...
        upsample_model3 = [torch.nn.ConvTranspose2d(454, ngf, #ngf*8
                                        kernel_size=3, stride=2,
                                        padding=1, output_padding=1,
                                        bias=use_bias),
                    norm_layer(ngf),
                    torch.nn.ReLU(True)]
        upsample_model4 = [torch.nn.ConvTranspose2d(288, ngf,
                                        kernel_size=3, stride=2,
                                        padding=1, output_padding=1,
                                        bias=use_bias),
                    norm_layer(ngf),
                    torch.nn.ReLU(True)]
        # upsample_model5 = [torch.nn.ConvTranspose2d(72, int(ngf/4),
        ...
        conv1_model = []
        conv1_model += [torch.nn.ReflectionPad2d(3)]
        conv1_model += [torch.nn.Conv2d(224, ngf, kernel_size=7, padding=0)]
        conv1_model += [torch.nn.ReLU(True)]
        conv1_model += [torch.nn.ReflectionPad2d(3)]
        conv1_model += [torch.nn.Conv2d(ngf, ngf, kernel_size=7, padding=0)]
        conv1_model += [torch.nn.ReLU(True)]
        ...
        self.layer1 = torch.nn.Linear(95, 4096)
        self.model_g128 =torch.nn.Sequential(*model_g_128)
        self.model_g64 =torch.nn.Sequential(*modelg_1)
        self.model_g32 =torch.nn.Sequential(*modelg_2)
        self.model_g32_ = torch.nn.Sequential(*modelg_2_)
        ...
        self.upsample_model3 = torch.nn.Sequential(*upsample_model3)
        self.upsample_model4 = torch.nn.Sequential(*upsample_model4)
        # self.upsample_model5 = torch.nn.Sequential(*upsample_model5)
        self.conv1_model = torch.nn.Sequential(*conv1_model)
        #self.convstyle = torch.nn.Sequential(*convstyle)
        ...
        self.conv_model_s = torch.nn.Sequential(*conv_model_s)
        self.upsample_patch1 = torch.nn.Sequential(*upsample_patch1)
        self.upsample_patch2 = torch.nn.Sequential(*upsample_patch2)

    def forward(self, input, global_input, style_global, input_pairs, pose, cross):
        # global input
        ...
        #style_global = self.convstyle(style_global)
        pose_ =  self.layer1(pose)
        pose = pose_.view(-1,1,64,64)
        ...
        for patch_c, patch_s in zip(patch_50_contents,patch_50_styles):
            ...
            conv_patch_s = self.conv_model_s(patch_s)
            conv_patch_cs = self.conv_patch_pair(patch_cs)
            pixel_patch_cs = torch.cat((conv_patch_cs,pose), dim=1)
            trans_patch_cs =  self.patch_transformer(pixel_patch_cs)
            trans_patch_cs = torch.cat((trans_patch_cs, conv_patch_s), dim=1)
            trans_patch_cs_128 = self.upsample_patch1(trans_patch_cs)
            trans_patch_cs_256 = self.upsample_patch2(trans_patch_cs_128)
            patch_feat_64.append(trans_patch_cs)
            patch_feat_128.append(trans_patch_cs_128)
            patch_feat_256.append(trans_patch_cs_256)
        patch_trans_feat = torch.cat(tuple(patch_feat_64),dim=1)# 64*12
        patch_trans_feat_128 = torch.cat(tuple(patch_feat_128),dim=1)# 64*12
        patch_trans_feat_256 = torch.cat(tuple(patch_feat_256),dim=1)# 64*12
        if cross:
            # print('use cross patch')
            patch_trans_feat_c = torch.cat((patch_trans_feat, global_64),dim=1)
            patch_trans_feat = self.cross_transformer(patch_trans_feat_c)
            #patch_trans_feat_all = torch.cat((patch_trans_feat_, patch_trans_feat),dim=1)
            #patch_global_feat = torch.cat((patch_trans_feat_all, style_global),dim=1)
        ...
        upsample_x_cat_g32 = torch.cat((upsample_x, global_32), dim=1)
        upsample_x64 = self.upsample_model2(upsample_x_cat_g32) 
        upsample_x_cat_g64 = torch.cat((patch_trans_feat, upsample_x64), dim=1)
        upsample_x128 = self.upsample_model3(upsample_x_cat_g64) 
        upsample_x_cat_g128 = torch.cat((patch_trans_feat_128, upsample_x128, global_128), dim=1)
        if flag==0:
            x_out = self.conv1_model(upsample_x_cat_g128)
        else:
            upsample_x256 = self.upsample_model4(upsample_x_cat_g128)
            #upsample_x_cat_g256 = upsample_x256
            upsample_x_cat_g256 = torch.cat((patch_trans_feat_256, upsample_x256, global_256), dim=1)
            x_out = self.conv1_model(upsample_x_cat_g256)
        return x_out, patch_feat_64
