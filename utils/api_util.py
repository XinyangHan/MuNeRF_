...
class FacePPAPI():
    ...
    def faceLandmarkDetector(self, filepath):
        ...
        '''for i in range(10):
            try:
                ...
                continue'''
        if 'faces'not in res_dict.keys() or not res_dict['faces'] or res_dict['face_num']==0:
            print('fail to get a landmark, reasons: no face or network diconnection.')
            return None
        else:
            return res_dict
    ...
    def getLandmarkByName(self, res_dict, name):
        result = []
        if 'faces' not in res_dict.keys():
            print('fail to get a landmark', res_dict)
            return None
        else:
