...
from nerf.load_flame import load_flame_data, load_flame_data_color_multi
from nerf.load_mask import load_mask_data2, mask_for_gt,load_mask_data_multi
...
from nerf import (CfgNode, get_embedding_function, get_ray_bundle, img2mse, make_new_get, make_new_get2,
                   meshgrid_xy, models, landmark2d_loss, cal_histloss_multi, prnet_multi, 
...
def main():
    ...
    if hasattr(cfg.dataset, "cachedir") and os.path.exists(cfg.dataset.cachedir):
        ...
        if cfg.dataset.type.lower() == "blender":
            if density:
                ...
                images, poses, render_poses, hwf, i_split, expressions, depths, bboxs, warped_imgs, makeups, paths, masks, img_ids= load_flame_data_color_multi(
                    cfg.dataset.basedir,
    ...
    for i in trange(start_iter, train_iters):
        ...
        if USE_CACHED_DATASET:
            ...
            img_target_lip = images[0][img_idx].to(device) # training target
            img_target_eyes = images[1][img_idx].to(device) # training target
            img_target_skin = images[2][img_idx].to(device) # training target
            ...
            if not density:
                ...
                img_B_arr_lip = np.array(torchvision.transforms.ToPILImage()((img_target_lip.permute(2, 0, 1)).detach().cpu()))
                landmark_B_api = paths_warped[img_idx]
                landmark_gt = fc.facePointsStasm(img_B_arr_lip)
                if landmark_B_api is not None:
                    img_target_rects = facePartsCoordinatesAPI(img_B_arr_lip, landmark_B_api, n_local=n_local, general_ratio = 0.1, scaling_factor=resize_scale)
                    img_target_rects_1 = facePartsCoordinatesAPI(img_B_arr_lip, landmark_B_api, n_local=n_local, general_ratio = 0.2, scaling_factor=resize_scale)
                    img_target_rects_2 = facePartsCoordinatesAPI(img_B_arr_lip, landmark_B_api, n_local=n_local, general_ratio = 0.4, scaling_factor=resize_scale)
                else:
                    img_target_rects = None # b
                
                # A dynamic makeup image

                makeup_target_lip = makeups[0][img_idx].to(device)
                makeup_target_eyes = makeups[1][img_idx].to(device)
                makeup_target_skin = makeups[2][img_idx].to(device)
                mask_makeup =  masks[1][img_idx].to(device)
                img_M_arr = np.array(torchvision.transforms.ToPILImage()((makeup_target_lip.permute(2, 0, 1)).detach().cpu()))
                ...
                img_target_t_lip, _ = cast_to_image2(img_target_lip[..., :3])
                img_target_t_eyes, _ = cast_to_image2(img_target_eyes[..., :3])
                img_target_t_skin, _ = cast_to_image2(img_target_t_skin[..., :3])

                makeup_target_t_lip, _ = cast_to_image2(makeup_target_lip[..., :3])
                makeup_target_t_eyes, _ = cast_to_image2(makeup_target_eyes[..., :3])
                makeup_target_t_skin, _ = cast_to_image2(makeup_target_skin[..., :3])
            ...
            if not density and cfg.loss.addloss.loss and i % cfg.loss.addloss.iter == 0:
                ...
                if save_res_debug and (i % 1 == 0):
                    ...
                    imgs = ([img_target_t_lip[0].unsqueeze(0),img_target_t_eyes[1].unsqueeze(0),img_target_t_skin[2].unsqueeze(0), img_target_t_skin].unsqueeze(0),real_fine,img_gt_t.unsqueeze(0))
                ...
                img_target = [img_target_lip, img_target_eyes, img_target_skin]
                makeup = [img.permute(2, 0, 1) for img in img_target]# first order image style_target; warp image img_target
                makeup_target = [makeup_target_lip,makeup_target_eyes,makeup_target_skin]
                makeup_gt = [img.permute(2, 0, 1) for img in makeup_target] # makeup image
                fake_makeup1, fake_makeup2, mask_no_makeup, mask_makeup, makeup, nonmakeup,makeup_gt = load_mask_data_multi(fake_makeup1,fake_makeup2,makeup,makeup_gt, img_gt_t, mask_no_makeup,mask_makeup, img_target_rects_1, resize_scale)
                if use_histogram_loss:
                    if use_old_histogram:
                        ...
                        histoloss_fine = cal_histloss_multi(criterionHis, vggnet, prn, fake_makeup1,makeup,makeup_gt, mask_no_makeup,mask_makeup,nonmakeup,debug_dir, size_img,if_dense)
                        histoloss_coarse = cal_histloss_multi(criterionHis, vggnet, prn, fake_makeup2,makeup, makeup_gt, mask_no_makeup,mask_makeup,nonmakeup,debug_dir, size_img,if_dense)
                        ###
                        hist_loss = b*histoloss_fine + a*histoloss_coarse
                        loss_hist = hist_loss * cfg.loss.histogram.histogram_weight
                        print('hist_loss', loss_hist)
                ...
                _, uv_fake_t, uv_target_t = prnet_multi(prn, debug_dir, fake_makeup1, makeup, makeup_gt, if_points, if_texture)
                ...
                if use_patchgan_loss and img_target_rects_1 is not None:
                    ...
                    if not if_uv:
                        ...
                        uv_target_t = [item.permute(2, 0, 1).unsqueeze(0) for item in uv_target_t]
                        image_a_coarse = [uv_fake_t.permute(2, 0, 1).unsqueeze(0), uv_fixed_rects]      #GEN
                        image_b = [uv_target_t, uv_fixed_rects] #!!!!!!!!!!!! #STYLE
                    coarsestyle_loss_g_dict = patchgan.local_style_g_multi(image_a_coarse, image_b, size_img, if_content)
                    ...
                    if not if_uv and fine_img is not None:
                        ...
                        finestyle_loss_g_dict = patchgan.local_style_g_multi(image_a_fine, image_b, size_img, if_content)
                    ...
                    if dis_back:
                        coarsestyle_loss_d_dict = patchgan.local_style_d_multi(image_a_coarse, image_b, size_img, if_content)
                        coarse_loss_d = sum(coarsestyle_loss_d_dict.values())
                        fine_loss_d = None
                        if not if_uv and fine_img is not None:
                            image_a_fine = [img_fine_t.unsqueeze(0), img_target_rects_1]
                            finestyle_loss_d_dict = patchgan.local_style_d_multi(image_a_fine, image_b, size_img, if_content)
                ...
                use_depth_patchgan =  False  
                # use content patchgan
                if use_patchgan_content_loss and img_target_rects_1 is not None:
                    if_content = True
                    size_img = 512 * cfg.dataset.scale
                    if use_depth_patchgan:
                        image_a_coarse = [depth_coarse_t.unsqueeze(0), img_target_rects_1]      #GEN
                        image_b = [depth_gt_t.permute(2, 0, 1).unsqueeze(0), img_target_rects_1]                  #STYLE
                    else:
                        image_a_coarse = [img_coarse_t.unsqueeze(0), img_target_rects_1]      #GEN
                        image_b = [img_gt.permute(2, 0, 1).unsqueeze(0), img_target_rects_1]                  #STYLE
                    coarsecontent_loss_g_dict = content_patchgan.local_style_g(image_a_coarse, image_b, size_img, if_content)
                    coarse_loss_g = sum(coarsecontent_loss_g_dict.values())
                    fine_loss_g = None
                    if fine_img is not None:
                        if use_depth_patchgan:
                            image_a_fine = [depth_fine_t.unsqueeze(0), img_target_rects_1]
                        else:
                            image_a_fine = [img_fine_t.unsqueeze(0), img_target_rects_1]
                        finecontent_loss_g_dict = content_patchgan.local_style_g(image_a_fine, image_b, size_img, if_content)
                        fine_loss_g = sum(finecontent_loss_g_dict.values())
                    patchgan_loss_g = coarse_loss_g + (fine_loss_g if fine_loss_g is not None else 0.0)
                    
                    if i>cfg.loss.patchgan.patchgan_iter + start_iter:
                        loss_patchgan_content = patchgan_loss_g*cfg.loss.patchgan.patchgan_g_content_weight1 \
                              if i<cfg.loss.patchgan.patchgan_iter + start_iter + cfg.loss.patchgan.patchgan_iter_weight \
                              else patchgan_loss_g*cfg.loss.patchgan.patchgan_g_content_weight2
                        print('patchgan c g, loss:', loss_patchgan_content)
                    else:
                        loss_patchgan_content = torch.tensor(0.0)
                        
                    loss_total = loss_total + loss_patchgan_content.to(device)
                    
                    if dis_back:
                        coarsecontent_loss_d_dict = content_patchgan.local_style_d(image_a_coarse, image_b, size_img, if_content)
                        coarse_loss_d = sum(coarsecontent_loss_d_dict.values())
                        fine_loss_d = None
                        if fine_img is not None:
                            image_a_fine = [img_fine_t.unsqueeze(0), img_target_rects_1]
                            finecontent_loss_d_dict = content_patchgan.local_style_d(image_a_fine, image_b, size_img, if_content)
                            fine_loss_d = sum(finecontent_loss_d_dict.values())
                        patchgan_loss_d = coarse_loss_d + (fine_loss_d if fine_loss_d is not None else 0.0)
                        loss_d_total = loss_d_total + patchgan_loss_d.to(device)
                        print('patchgan c d, loss:', patchgan_loss_d)
            ...
            with torch.no_grad():
                ...
                if USE_CACHED_DATASET:
                    ...
                    for img_idx in i_val[:2]:
                        ...
                        target_ray_values = img_target_skin
