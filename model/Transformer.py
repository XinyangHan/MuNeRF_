...
def define_G(embed_dim, netG, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    if netG == 'TransEncoder':
        net = ResnetGeneratorTransGenerator(embed_dim)
    else:
...
class ResnetGeneratorTransGenerator(nn.Module):
    ...
    def forward(self, input_f):
        x = input_f.view(-1, input_f.shape[1], input_f.shape[2]*input_f.shape[3]) #1,3,576
        for index, blk in enumerate(self.TransformerBlock):
            x = blk(x)
        x = x.view(-1, input_f.shape[1], input_f.shape[2], input_f.shape[3])  #1,3,24,24
        return x
