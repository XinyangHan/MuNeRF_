...
class dataset_makeup(data.Dataset):
    def __init__(self, opts, mode ='train'):
        ...
        #blend images
        imgs_blend = os.listdir(os.path.join(self.dataroot, "blend"))
        self.C_all = [os.path.join(self.dataroot, "blend", x) for x in imgs_blend]
        self.C = self.C_all
        self.C.sort(key=lambda x:int(x.split('/')[-1][:4])) #(key=lambda x:int(x[13:15])
        #print('path_C',self.C)
        ...
        if self.mode == "test":
            ...
            if opts.test_random:
                ...
                for i in range(self.test_size):
                    ...
                    #print('TEST_path_C', path_C)
                    self.datapoints.append([path_A, path_B, path_C])
                    print('len of datapoint', len(self.datapoints))
                random.setstate(rs)
            else:
                print("Iterating over all pairs of images and generate results")
                for index in range(len(self.C)):
                    path_C = self.C[index]
                    path_A = os.path.join(self.dataroot, "before", path_C.split('/')[-1][:5]+'.jpg')
                    #path_C = self.getPathC(path_A, path_B)
                    path_B = os.path.join(self.dataroot, "after", path_C.split('/')[-1][6:11]+'.jpg')
                    self.datapoints.append([path_A, path_B, path_C])
        ...
        # normal training dataset
            ...
            for index in range(len(self.C)):
                path_C = self.C[index]
                path_A = os.path.join(self.dataroot, "before", path_C.split('/')[-1][:4]+'.png')
                #path_C = self.getPathC(path_A, path_B)
                path_B = os.path.join(self.dataroot, "after", path_C.split('/')[-1][5:10]+'.jpg')
                self.datapoints.append([path_A, path_B, path_C])
                #print('self.datapoints', self.datapoints)
                #random.shuffle(self.datapoints)
            self.datapoints = self.datapoints[:self.test_size]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "test":
            ...
            #print('path_of_A_B_C', path_A, path_B, path_C)
        elif self.mode == "interpolate":
            ...
            '''if self.dataset_size == self.A_size:
                ...
                path_B = self.B[index]
            path_C = self.getPathC(path_A, path_B)'''
            print('current training datapoint',index,'\n', path_A,path_B,path_C)
        else:
