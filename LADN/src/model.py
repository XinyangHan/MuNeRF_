...
class LADN(nn.Module):
    ...
    def backward_G_GAN_content(self, z_content):
        ...
        for out in outs:
            ...
            loss_G = loss_G + nn.functional.binary_cross_entropy(outputs_fake, all_half)
        return loss_G
    ...
    def resume(self, model_dir, train=True):
        ...
        #print('back_checkpoint',checkpoint_backup)
    ...
    def assemble_outputs(self):
        images_a = self.normalize_image(self.real_A_encoded).detach().to(self.device)
        images_b = self.normalize_image(self.real_B_encoded).detach().to(self.device)
        images_a1 = self.normalize_image(self.fake_A_encoded).detach().to(self.device)
        images_a2 = self.normalize_image(self.fake_A_random).detach().to(self.device)
        images_a3 = self.normalize_image(self.fake_A_recon).detach().to(self.device)
        images_a4 = self.normalize_image(self.fake_AA_encoded).detach().to(self.device)
        images_b1 = self.normalize_image(self.fake_B_encoded).detach().to(self.device)
        images_b2 = self.normalize_image(self.fake_B_random).detach().to(self.device)
        images_b3 = self.normalize_image(self.fake_B_recon).detach().to(self.device)
        images_b4 = self.normalize_image(self.fake_BB_encoded).detach().to(self.device)
        images_c = self.normalize_image(self.real_C_encoded).detach().to(self.device)
        
        #print('images_b,images_a1,image_b2,imagea4,imagea3', images_a, images_b1, images_b2, images_a4, images_a3)
