...
if __name__ == '__main__':
    ...
    parser.add_argument('--frame_dir', type=str, default='', help='path to the frame dir')
    parser.add_argument('--video_dir', type=str, default='', help='path to the video dir')
    parser.add_argument('--video',action="store_true", help='true for pic2video')
    args = parser.parse_args()
    inputpath = args.frame_dir
    outputpath = os.path.join(args.video_dir, args.id+'.mp4')
    video = args.video
    if not video: Video2Pic(outputpath, inputpath)
    if video: Pic2Video(inputpath, outputpath)
